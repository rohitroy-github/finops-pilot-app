...
export async function planner(
  ...
  eventLogger.info("New incident receievd - planner activated.", {
    stage: "planner_start",
    dependency: normalizedEvent.dependency,
    billing_url: normalizedEvent.billing_url,
  });

  eventLogger.info("Launching browser context for merchant pricing page to scrape data.", {
  ...
  eventLogger.info("Scraping merchant pricing page.", { stage: "scraping" });
  const pricingText = await extractPricingText(page);
  eventLogger.info("Merchant pricing page scrapped successfully.", {
    stage: "scrape_complete",
    scrapedTextLength: typeof pricingText === "string" ? pricingText.length : 0,
  });

  await page.context().close();
  eventLogger.info("Scrapping done, closing browser context.", { stage: "browser_closed" });
  ...
  eventLogger.info("Fetched LLMs analyzed data successfully.", {
    stage: "llm_response",
  ...
  eventLogger.success(
    `Analyzed LLM api plan: ${purchasePlan.recommendedPlan?.name} @ ₹${purchasePlan.exactCost}`,
    {
      stage: "llm_decision",
      plan: purchasePlan.recommendedPlan?.name,
  ...
  eventLogger.info(`Agent job created = id: ${job.id}`, {
    stage: "job_created",
    jobId: job.id,
  });
  ...
  try {
    eventLogger.info(`Creating Prava payment session for job@${job.id}.`, {
    ...
    eventLogger.success("Prava session initialized, waiting for your payment approval.", {
  ...
  const merchantCheckoutUrl =
    process.env.LIVE_MERCHANT_CHECKOUT_URL ??
    "https://translate-ai-kappa.vercel.app/checkout?plan=Developer&client_id=client_auth_12345";

  let merchantPaymentOutcome: MerchantPaymentWatcherResult;

  if (pravaSession?.sessionId) {
    eventLogger.info("Watching for Prava generated credentials.", {
  ...
  eventLogger.success("Planner executed successfully.", {
...
//   const merchantCheckoutUrl =
//     process.env.DEMO_MERCHANT_CHECKOUT_URL ??
